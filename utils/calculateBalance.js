...
export const calculateReferralBalance = (userDetails) => {
  ...
  //Calculate total referral earnings and get the referral withdrawals
  const totalEarnings = sumAmounts(userDetails.Earnings);
  const totalReferralWithdrawals = sumAmounts(userDetails.ReferralWithdrawals)
  const balance = totalEarnings - totalReferralWithdrawals;
  return balance;
}